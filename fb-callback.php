...
$query=$db->prepare('SELECT * FROM uzivatele WHERE facebook_id=:facebookId LIMIT 1;');
$query->execute([
    ':facebookId'=>$fbUserId
]);
...
if ($query->rowCount()>0){
    ...
    if ($query->rowCount()>0){
        ...
        $updateQuery = $db->prepare('UPDATE uzivatele SET facebook_id=:facebookId WHERE id=:id;');
        $updateQuery->execute([
            ':facebookId'=>$fbUserId,
            ':id'=>$user['id']
        ]);
        ...
        $insertQuery = $db->prepare('INSERT INTO uzivatele (name, email, facebook_id) VALUES (:name, :email, :facebookId);');
        $insertQuery->execute([
            ':name'=>$fbUserName,
            ':email'=>$fbUserEmail,
            ':facebookId'=>$fbUserId
